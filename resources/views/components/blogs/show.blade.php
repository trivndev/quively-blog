<article class="format format-sm sm:format-base lg:format-lg format-blue dark:format-invert mx-auto w-full max-w-7xl">
    <header class="not-format mb-4 space-y-4 lg:mb-6">
    ...
    <div id="content">
        <h4 class="w-full break-words text-3xl font-extrabold leading-none text-gray-900 lg:text-4xl dark:text-white">
            {{ $post->title }}</h4>
        <a href="/blog?category={{ $post->category->slug }}" title="Go to {{ $post->category->name }} category"
            class="{{ $post->category->color }} dark:bg-primary-200 dark:text-primary-800 hover:bg-primary-200 dark:hover:bg-primary-300 mb-2 block w-fit items-center rounded px-2.5 py-2 text-xs font-medium text-gray-800 no-underline transition-all duration-300 ease-in-out hover:text-black">
            {{ $post->category->name }}
        </a>
